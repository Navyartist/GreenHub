...
class bookShelf():
    def __init__(self, shelf_name):
        ...
        self.file_path = os.path.join(current_dir, f'data_{shelf_name}.json')
        # 진짜 쓸 파일패스(파일경로)
...
# 하고싶은 거 1: 책검색 메서드에서 사용자가 없는 필드를 쓰면 오류났다며 함수 종료시키는 루틴 추가
# 하고싶은 거 2(구현완료): 사용자가 종료하고 싶을 때까지 무한루프하는 기능
# 문제 1: 책정보 바꾸기()
#   책이름이 너무 길거나 복잡하면 쓰기가 힘든데, 수정할 책을 선택할 때 ISBN같은 고유코드를 사용해야 할까? +이름이 똑같은 책일때도 문제, 책치우기 메서드에도 같은 맥락
# 문제 2(해결완료): 이 코드에 class 쓰려면? -클래스화, 메인/모듈 파일 분리함
# 하고싶은 거 3: 태그를 여러개 쓰고 싶음
